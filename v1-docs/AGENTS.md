...
不得擅自开发轨迹实时、轨迹回放、统计报告（不含已确认的流量统计）、用户管理、完整乘客动态（不含已确认的智慧舷窗展示）、推荐、PHM、ACARS 或视频业务。需要新增范围时，先更新 `SPEC.md` 并确认。
...
- 首期 Schema 固定为 18 张表：8 张公共表、上述 7 张模拟器业务表，加 `traffic_stat_snapshot`、`cabin_window_layout`、`smart_window_current_status` 3 张展示派生表。未经 `SPEC.md` 变更不得增加其他业务表。
...
经负责人确认，Phase 2 只允许开发“乘客实时动态”单个只读页面。参与者开始相关开发前必须依次阅读 `phase2-contracts/SPEC.md`、`phase2-contracts/API.md`、`phase2-contracts/schema.md`、`phase2-contracts/SLICES.md` 和 `phase2-contracts/AGENTS.md`。

上述文件仅在该页面范围内覆盖根级文档的“暂不开发乘客实时动态”限制；“数据统计”仍不在本期范围。登录、数据管理、导入导出及其他功能继续以根级契约为准。驾驶舱实时监控只允许保留无接口的静态面板；不得扩展流量统计、统计报告、乘客推荐、驾驶舱视频接口、舷窗控制或用户管理功能。
