...
- 实现筛选、航段列表、轨迹加载和四态页面。
- 实现地图已飞轨迹、飞机定位、状态卡、图表游标、固定 10 分钟滚动横轴、图表展示点降采样、进度条、播放/暂停、回到起点和默认 5×的 1×、2×、5×、10×、20× 五档倍速。

验证：进度拖动、每种倍速、窗口连续左移、历史时间桶稳定、未来点隐藏、各档展示点上限、切换航段、离开页面停止播放、后台错误保留上次成功数据、1366 与 1920 宽度布局。

## 8. 切片 6：联调与验收

目标：

- 模拟器现可生成起飞、巡航、下降、单点落地和连续换航数据；无停场连续换航用于验证 `NEW_FLIGHT` 收尾，严格 `LANDED` 仍需连续 5 分钟地面测试夹具。
