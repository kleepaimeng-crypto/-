...
| 6 | 分类型 CSV 导入 | 模板、导入任务和错误报告可用 | Slice 3、5 |
| 7 | CSV/PDF 导出 | 筛选结果可异步导出并下载 | Slice 4、5 |
| 8 | 联调、保留策略与验收 | 首期验收清单全部通过 | Slice 1–7 |
...
- `schema.md` 恰好定义 15 张首期表：8 张公共表和 7 张模拟器业务表。
...
- 按 `schema.md` 建立 15 张表、外键、检查约束、索引和种子数据。
- 禁用 ORM 自动建表。
