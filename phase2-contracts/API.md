...
- Controller 只负责参数绑定、校验和统一响应；三个端点均调用同一乘客实时动态查询服务。
...
- 在集中 API 模块中定义 `PassengerRealtimeSnapshotDto`、`PassengerActivityDto`、`PassengerActivityPageDto` 和 `SeatLocationDto`，禁止使用 `any`。
- 暴露 `getPassengerRealtimeSnapshot()`、`getPassengerActivities(page)` 和 `locatePassengerActivity(seatNo)` 三个调用函数。
...
- 前端依据 `windowId` 派生位置：1–58 为左侧、59–116 为右侧；同侧序号 1–17 前舱、18–37 中舱、38–58 后舱。
...
1. 前端并行请求 `snapshot` 和 `passenger-activities?page=1&pageSize=4`。
2. 快照负责更新排行、座位状态和舷窗；分页响应只更新右侧四条详情。
...
- 刷新当前详情页，不重置 `page`、`seatNo`、高亮项或舱位图 `scrollTop`。
