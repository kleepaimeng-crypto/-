...
- 当前航班采用现有 UDP 当前飞行上下文的 `flight_no`，其权威来源为 QAR；不得以最新科可瑞尔事件替代该判断。
- 科可瑞尔事件复用当前飞行上下文补齐 `data_record` 的 `origin`、`destination` 和 `airline_code` 等管理字段；其自身 `flight_no` 始终来自客户 `flightId`。
- 当 IFE `flight_no` 与当前飞行上下文不一致时，事件仅保留在历史表和其所属航班的当前状态表行中，不得出现在当前 QAR 航班的快照。
