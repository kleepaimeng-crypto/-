...
- `flightSessionId + seatNo` 是当前状态唯一键；`flightSessionId` 仅来自当前匹配的 QAR 会话，不得只用 `flightId`、`passengerId` 或 `seatNo`。
- 科可瑞尔必须复用现有 UDP 当前飞行上下文；QAR 是当前飞行判断及航线信息的权威来源。
- IFE 的 `flightId` 与当前 QAR 会话不一致，或 QAR 会话不存在时，只保存该事件历史，不得由 IFE 更新缓存或切换全局飞行上下文。
- 对航班号匹配的事件，仍须校验事件时间不早于 QAR 会话 `startedAt - 5 分钟`；不通过时只保存历史。
...
- 已确认的 `playAction = PLAY`、`PAUSE` 是已收到的真实乘客操作，必须保留其影音行为与操作值，不能显示为空闲；未经客户确认不得模拟 `STOP`。
- 每条新 KKRE 单事件仍对应一条 `data_record` 和一条 `ife_cockrell_behavior`，固定 `item_no = 1`，以支持数据管理与审计；不改变历史表既有的批量约束。
- 当前状态缓存只保存在后端内存，不能替代历史事件表；服务重启或缓存缺失时不重建，等待新的匹配事件。
- PNR、IP、完整订单、原始报文和封面 Base64 不得默认暴露给乘客实时接口。

## 4. 数据库规则

- 本期不新增 Flyway 迁移，不清理旧数据，不移除 `item_no`；禁止修改已发布迁移。
- `ife_cockrell_behavior` 保持追加式历史表，不新增会覆盖历史的唯一约束。
- 历史事件事务提交成功后才更新内存缓存；持久化失败不得改变缓存。
- 内存缓存必须限制旧事件不覆盖新事件，且处理相同事件时间的稳定顺序。
- 282 座位突发写入必须按独立缓存键安全处理；不得先清空整航班状态再重建。
- QAR 会话结束或切换时必须立即删除旧会话缓存，防止内存积累与跨会话展示。
...
- 本期乘客实时快照只使用 KKRE 缓存，不得与 633 IFE 合并；633 接收和协议不变。
...
- 每个航段的首次 KKRE 发送连续发出 282 个独立事件；同一航段后续默认随机发送单座位事件。
- 突发场景通过短时间连续发送多个独立 UDP 包实现，不能用一个数组替代。
- 模拟器保持现有 QAR 优先的定时任务发送顺序，不增加上下文就绪等待或握手；不得把上下文字段添加到客户固定 JSON。
- 模拟器必须覆盖已确认的 `playAction = PLAY`、`PAUSE`，并验证其显示为真实操作而非空闲；不得自行扩展 `STOP`。
- 保持固定随机种子和可复现测试。
